...
style frame:
    ...
    background At(Frame("gui/frame.webp", gui.frame_borders, tile=gui.frame_tile))
...
transform saybounce:
    on show:
        ...
        ease 0.05  yoffset 0
    on hide:
        ease 0.05 zoom 0.98 yoffset 2 alpha 0.0
...
##
## https://www.renpy.org/doc/html/screen_special.html#sa
default window_bg=False
screen say(who, what):
    style_prefix "say"
    zorder 105
    window:
        ...
        anchor (0,0) ## Top left corner of the inner area
        xmargin 110 ## to account for the page border
        ymargin 110
        vbox:
            if who is not None:
                window:
    ...
    ## If there's a side image, display it above the text unless phone.
...
style window:
    pos (gui.textbox_xpos, gui.textbox_ypos)
    xsize gui.textbox_width+480
    ysize gui.textbox_height+240
    padding (15, 15, 80, 60)
    # background Frame("images/r_sqww.webp",left = Borders(30, 30, 30, 30))
style say_dialogue:
    ...
    # xalign 0.5
    xsize 800                                    # gui.textbox_width-120 #gui.dialogue_width
...
## When TRUE, menu captions will be spoken by the narrator.
## When FALSE, menu captions will be displayed as empty buttons.
...
## Navigation screen ###########################################################
##
## In the main and game menus, and provides navigation to other menus, and to start the game.
    ...
    hbox:
        ...
        for i in ["p"]+range(1,6)+["e"]:
            button:
                ...
                hover_background Solid("a5a5a5")
                # xalign 0.0 yalign 0.88
                text "{b}{size=40}[i]{/size}{/b}" xalign 0.5 yalign 0.5
                action Start("startchapter"+str(i))
            null width 10
        ...
        #     background "images/w_sq.webp"
        #     # xalign 0.0 yalign 0.88
        #     text "{size=50}3{/size}" xalign 0.5 yalign 0.5
        # frame:
        #     ysize 88
        #     xsize 88
        #     background "images/w_sq.webp"
        #     # xalign 0.0 yalign 0.88
        #     text "{size=50}4{/size}" xalign 0.5 yalign 0.5
        # frame:
        #     ysize 88
        #     xsize 88
        #     background "images/w_sq.webp"
...
screen main_menu():
    ...
    frame:
        ...
        xalign 0.85
        yalign 0.32
        # text "Monochrome\nValentine" size 130 text_align 0.9:
        #     color "#fff"
        #     outlines [(absolute(5), "#707070", absolute(1), absolute(1))]
        #     bold True
    frame:
        style "mm_start"
        background None
        xalign 0.0
        yalign 1.0
        # imagebutton auto "gui/blcorner_%s.webp" action Start()
    frame:
        style "mm_start"
        xanchor 0.0
        yanchor 1.0
        xpos 0.15
        ypos 0.99
        # textbutton _("<-- Start Reading") action Start() style "mm_start"
    frame:
        style "mm_start"
        background None
        xalign 0.0
        ...
        xmaximum 175
        ymaximum 175
        # imagebutton idle "gui/bookmark.webp" action ShowMenu("load")
    ## Quit Button
    if renpy.variant("pc"):
        textbutton _("Quit") action Quit(confirm=not main_menu) style "mm_quit" at mm_quit_move

style mm_box:
    xfill True
    yalign 0.85
    ysize 220
    background Frame("gui/overlay/confirm.webp", gui.frame_borders, tile=gui.frame_tile)
transform mm_boxmove:
    xanchor 1.0
    xpos 0.0
    ease 0.5 xpos 1.0

style mm_start:
    background None #At(Frame("white", gui.frame_borders, tile=gui.frame_tile), alpha05)
style mm_start_text:
    size 45
    color "#e0e0e0"
...
style mm_tb_text:
     ...
     bold True
     hover_color "#9D9D9D"
     outlines [(absolute(4), "#000", absolute(2), absolute(2))]
transform mm_tb_move(delay):
    subpixel True
...
style game_menu_content_frame:
    bottom_padding 100 top_padding 100 left_padding 100 right_padding 100
    # left_margin 54 right_margin 27 top_margin 14
    # background At(Frame("gui/nvl.webp"), alpha05)
...
style return_button:
    xanchor 0.5 #xpos gui.navigation_xpos
    xpos 50
    yalign 1.0
    background None
    # yoffset -39
...
screen about():
    ...
    ## The vbox child is then included inside the viewport inside the game_menu screen.
...
##
## Both are implemented in terms of a third screen, file_slots.
##
        ...
        fixed:
            ...
            hbox:
                ...
                for page in range(1, 6):
                    textbutton "Page [page]" action FilePage(page)


            ## The grid of file slots. Right now 2x2 in gui
            grid gui.file_slot_cols gui.file_slot_rows:
                ...
                for i in range(gui.file_slot_cols * gui.file_slot_rows):
                    ...
                    button:
                        ...
                        hbox:
                            null width 20
                            add FileScreenshot(slot) yalign 0.0
                            null width 50
                            text FileTime(slot, format=_("{#file_time}%A, %B %d %Y, %H:%M"), empty=_("empty slot")):
                                style "slot_time_text"
                                yalign 0.5
                            null width 10
                        key "d" action FileDelete(slot)
...
## History screen ##############################################################
## it does have to access the dialogue history stored in _history_list.
##
...
## It uses other screens (keyboard_help, mouse_help, and gamepad_help).
...
screen keyboard_help():
    ...
    hbox:
        label "D"
        text _("Delete save file when hovered.")
    hbox:
        label "V"
